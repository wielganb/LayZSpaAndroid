...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView weatherTitle,weatherDetails,weatherRecommendation; private MaterialButton weatherRefresh,weatherSetButton,weatherLocationButton;
    ...
    private View buildMain(){
        ...
        root.setPadding(dp(8),dp(8),dp(8),dp(24));
        sc.addView(root);

        // HEADER — lekki i zbliżony do ustalonego projektu.
        MaterialCardView head=card();
        LinearLayout hb=inside(head,dp(10));
        ...
        TextView title=tv("Smart Spa",26,TEXT);
        title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        title.setGravity(Gravity.CENTER);
        title.setSingleLine(true);
        top.addView(title,new LinearLayout.LayoutParams(0,dp(58),1));
        status=tv("●  "+tr("disconnected"),13,RED);
        status.setGravity(Gravity.CENTER);
        status.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        top.addView(status,new LinearLayout.LayoutParams(0,dp(52),1));
        languageSpinner=spinner(FLAGS,langIndex());
        LinearLayout.LayoutParams lsp=new LinearLayout.LayoutParams(dp(118),dp(52));
        lsp.setMargins(dp(6),0,0,0);
        ...
        LinearLayout tb=inside(tempCard,dp(14));
        tempCaption=cap(tr("currentTemp"));
        tempCaption.setGravity(Gravity.CENTER);
        tb.addView(tempCaption,new LinearLayout.LayoutParams(-1,dp(30)));

        dial=new TemperatureDial();
        tb.addView(dial,new LinearLayout.LayoutParams(-1,dp(360)));

        targetValue=tv(tr("target")+" 30°C",30,TEXT);
        targetValue.setGravity(Gravity.CENTER);
        targetValue.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        LinearLayout.LayoutParams targetLp=new LinearLayout.LayoutParams(-1,dp(56));
        targetLp.setMargins(0,dp(4),0,dp(6));
        tb.addView(targetValue,targetLp);

        // SESJA — osobna, wyraźna sekcja zamiast zlepionej listy.
        MaterialCardView sessionCard=new MaterialCardView(this);
        sessionCard.setCardBackgroundColor(CARD2);
        sessionCard.setRadius(dp(18));
        sessionCard.setStrokeColor(BORDER);
        sessionCard.setStrokeWidth(1);
        LinearLayout sb=inside(sessionCard,dp(12));

        LinearLayout sessionHead=row();
        TextView sessionTitle=cap("SESJA SPA");
        sessionTitle.setGravity(Gravity.CENTER_VERTICAL);
        sessionHead.addView(sessionTitle,new LinearLayout.LayoutParams(0,dp(28),1));
        TextView lengthLabel=cap("DŁUGOŚĆ");
        lengthLabel.setGravity(Gravity.CENTER);
        sessionHead.addView(lengthLabel,new LinearLayout.LayoutParams(dp(116),dp(28)));
        sb.addView(sessionHead);

        LinearLayout session=row();
        session.setGravity(Gravity.CENTER_VERTICAL);
        sessionTimerView=tv("",28,TEXT);
        sessionTimerView.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        sessionTimerView.setGravity(Gravity.CENTER_VERTICAL);
        session.addView(sessionTimerView,new LinearLayout.LayoutParams(0,dp(58),1));
        sessionButton=bigButton("");
        sessionButton.setTextSize(14);
        sessionButton.setTypeface(Typeface.DEFAULT,Typeface.BOLD);
        sessionButton.setPadding(dp(12),0,dp(12),0);
        sessionButton.setOnClickListener(v->showSessionDurationDialog());
        LinearLayout.LayoutParams sessionBtnLp=new LinearLayout.LayoutParams(dp(116),dp(54));
        sessionBtnLp.setMargins(dp(8),0,0,0);
        session.addView(sessionButton,sessionBtnLp);
        sb.addView(session,new LinearLayout.LayoutParams(-1,dp(58)));

        gap(sb,dp(8));
        LinearLayout durationRow1=row();
        durationRow1.setPadding(dp(2),0,dp(2),0);
        LinearLayout durationRow2=row();
        durationRow2.setPadding(dp(2),0,dp(2),0);
        ...
        for(int i=0;i<durations.length;i++){
            ...
            db.setPadding(dp(2),0,dp(2),0);
            db.setMinWidth(0);
            db.setMinimumWidth(0);
            db.setOnClickListener(v->setSessionDuration(minutes));
            sessionQuickButtons[i]=db;
            LinearLayout.LayoutParams dpLp=new LinearLayout.LayoutParams(0,dp(44),1);
            dpLp.setMargins(dp(2),0,dp(2),0);
            if(i<4)durationRow1.addView(db,dpLp);else durationRow2.addView(db,dpLp);
        }
        sb.addView(durationRow1,new LinearLayout.LayoutParams(-1,dp(46)));
        gap(sb,dp(4));
        sb.addView(durationRow2,new LinearLayout.LayoutParams(-1,dp(46)));
        updateSessionQuickButtons();
        tb.addView(sessionCard,new LinearLayout.LayoutParams(-1,-2));
        gap(tb,dp(12));

        // Główne sterowanie — duże, równe przyciski jak na projekcie referencyjnym.
        LinearLayout quick=row();
        quick.setPadding(dp(0),0,dp(0),0);
        quickHeat=bigButton("♨  "+label("heat"));
        quickAir=bigButton("≈  "+label("bubbles"));
        quickPump=bigButton("◉  "+label("filtering"));
        LinearLayout.LayoutParams q1=new LinearLayout.LayoutParams(0,dp(58),1);q1.setMargins(dp(2),0,dp(2),0);
        LinearLayout.LayoutParams q2=new LinearLayout.LayoutParams(0,dp(58),1);q2.setMargins(dp(2),0,dp(2),0);
        LinearLayout.LayoutParams q3=new LinearLayout.LayoutParams(0,dp(58),1);q3.setMargins(dp(2),0,dp(2),0);
        quick.addView(quickHeat,q1);quick.addView(quickAir,q2);quick.addView(quickPump,q3);
        tb.addView(quick,new LinearLayout.LayoutParams(-1,dp(62)));
        ...
        gap(root,dp(12));
        root.addView(buildWeatherCard(),new LinearLayout.LayoutParams(-1,-2));
        gap(root,dp(12));
        return sc;
    }

    private View buildSettings(){ScrollView sc=new ScrollView(this);sc.setFillViewport(true);LinearLayout root=vertical();root.setPadding(10,10,10,30);sc.addView(root);
        MaterialCardView head=card();LinearLayout hb=inside(head,14);LinearLayout h=row();backButton=bigButton("‹   "+tr("back"));backButton.setOnClickListener(v->showMain());h.addView(backButton,new LinearLayout.LayoutParams(0,64,1));settingsTitle=tv(tr("spaSettings"),24,TEXT);settingsTitle.setGravity(Gravity.CENTER);settingsTitle.setTypeface(Typeface.DEFAULT,Typeface.BOLD);settingsTitle.setSingleLine(true);h.addView(settingsTitle,new LinearLayout.LayoutParams(0,64,2));hb.addView(h);root.addView(head,new LinearLayout.LayoutParams(-1,-2));gap(root,12);
        MaterialCardView controls=card();LinearLayout c=inside(controls,16);TextView ct=cap(tr("spaControl"));ct.setTextSize(13);c.addView(ct);gap(c,8);heatLabel=tv("",16,TEXT);pumpLabel=tv("",16,TEXT);airLabel=tv("",16,TEXT);jetsLabel=tv("",16,TEXT);powerLabel=tv("",16,TEXT);addSwitch(c,heatLabel,3);addSwitch(c,pumpLabel,4);addSwitch(c,airLabel,2);addSwitch(c,jetsLabel,11);addSwitch(c,powerLabel,25);root.addView(controls);gap(root,12);
        MaterialCardView spa=card();LinearLayout s=inside(spa,16);TextView st=cap(tr("spaSettings"));st.setTextSize(13);s.addView(st);gap(s,8);TextView pl=tv(tr("pumpProfile"),15,TEXT);pl.setTypeface(Typeface.DEFAULT,Typeface.BOLD);s.addView(pl);gap(s,5);pumpSpinner=spinner(new String[]{tr("default"),tr("pump1"),tr("pump2"),tr("external")},pumpIndex());s.addView(pumpSpinner,new LinearLayout.LayoutParams(-1,64));gap(s,12);TextView hl=tv(tr("hydroMode"),15,TEXT);hl.setTypeface(Typeface.DEFAULT,Typeface.BOLD);s.addView(hl);gap(s,5);hydroSpinner=spinner(new String[]{tr("standard"),tr("power"),tr("quiet")},hydroIndex());s.addView(hydroSpinner,new LinearLayout.LayoutParams(-1,64));gap(s,10);settingsNote=tv(tr("note"),13,MUTED);settingsNote.setLineSpacing(0,1.15f);s.addView(settingsNote);pumpSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onNothingSelected(AdapterView<?> p){}public void onItemSelected(AdapterView<?> p,View v,int pos,long id){pumpProfile=new String[]{"default","pump1","pump2","external"}[pos];saveSettings();}});hydroSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onNothingSelected(AdapterView<?> p){}public void onItemSelected(AdapterView<?> p,View v,int pos,long id){hydroMode=new String[]{"standard","power","quiet"}[pos];saveSettings();}});root.addView(spa);gap(root,12);root.addView(buildWeatherSettings());return sc;}
    ...
    private View buildWeatherSettings(){
        MaterialCardView c=card(); LinearLayout b=inside(c,16); TextView t=cap("Lokalizacja SPA"); b.addView(t); gap(b,5);
        weatherLocationButton=bigButton("📍  "+(weatherCity.isEmpty()?"Ustaw lokalizację":weatherCity)); b.addView(weatherLocationButton,new LinearLayout.LayoutParams(-1,68));
        weatherLocationButton.setOnClickListener(v->showWeatherLocationDialog()); gap(b,7);
        TextView n=tv("Pogoda jest pobierana dla miejsca, w którym znajduje się jacuzzi. Aplikacja nie musi korzystać z GPS telefonu.",13,MUTED); n.setLineSpacing(0,1.1f); b.addView(n);
        return c;
    }
    ...
    private void saveWeatherLocation(String city,double lat,double lon){weatherCity=city;weatherLat=lat;weatherLon=lon;getSharedPreferences("weather",0).edit().putString("city",city).putFloat("lat",(float)lat).putFloat("lon",(float)lon).apply();if(weatherLocationButton!=null)weatherLocationButton.setText("📍  "+city);fetchWeather();}
    private void showWeatherLocationDialog(){
        LinearLayout box=vertical(); box.setPadding(22,8,22,8); TextView hint=tv("Wpisz miejscowość, np. Gościęcin",14,MUTED); box.addView(hint); EditText q=new EditText(this); q.setSingleLine(true); q.setText(weatherCity); q.setTextSize(17); box.addView(q,new LinearLayout.LayoutParams(-1,62)); TextView result=tv("",15,TEXT); result.setLineSpacing(0,1.15f); box.addView(result); MaterialButton search=bigButton("SZUKAJ");box.addView(search,new LinearLayout.LayoutParams(-1,64));
    ...
    private void updateWeatherViews(){if(weatherDetails==null)return;if(weatherLat==0&&weatherLon==0){weatherDetails.setText("Nie ustawiono lokalizacji jacuzzi.\nWejdź w Ustawienia → Lokalizacja SPA.");weatherRecommendation.setText("Zalecana temperatura SPA: --");return;}weatherDetails.setText("📍 "+weatherCity+"\nPogoda: "+weatherText+"\nNa zewnątrz: "+outsideTemp+"°C");weatherRecommendation.setText("Zalecana temperatura SPA: "+recommendedTemp+"°C");if(weatherLocationButton!=null)weatherLocationButton.setText("📍  "+weatherCity);}
    ...
    private void addSwitch(LinearLayout parent,TextView label,int cmd){
        LinearLayout r=row();r.setPadding(12,4,8,4);r.setGravity(Gravity.CENTER_VERTICAL);r.setBackgroundResource(android.R.drawable.list_selector_background);r.setClickable(true);
        String icon=cmd==3?"♨":cmd==4?"◉":cmd==2?"≈":cmd==11?"✦":"⏻";
        TextView ico=tv(icon,22,TEXT);ico.setGravity(Gravity.CENTER);r.addView(ico,new LinearLayout.LayoutParams(42,64));
        label.setTextSize(16);label.setTypeface(Typeface.DEFAULT,Typeface.BOLD);label.setGravity(Gravity.CENTER_VERTICAL);label.setSingleLine(true);r.addView(label,new LinearLayout.LayoutParams(0,64,1));
        ...
        LinearLayout.LayoutParams slp=new LinearLayout.LayoutParams(82,48);slp.setMargins(6,0,0,0);r.addView(state,slp);
        ...
        label.setTag(state);parent.addView(r,new LinearLayout.LayoutParams(-1,70));gap(parent,5);
    }
    ...
    private void showNetworkConfig(){
        ...
        scan.setOnClickListener(v->{if(discoveryRunning)return;discoveryRunning=true;scan.setEnabled(false);state.setText(tr("scanning"));state.setTextColor(BLUE);discoveryExecutor.execute(()->{String found=discoverSpaIp();runOnUiThread(()->{discoveryRunning=false;scan.setEnabled(true);if(found!=null){ip.setText(found);host=found;getSharedPreferences("spa",0).edit().putString("host",host).apply();state.setText(tr("found_spa")+" "+found);state.setTextColor(GREEN);manual.run();}else{state.setText(tr("not_found"));state.setTextColor(MUTED);}});});});
        d.show();
    }
