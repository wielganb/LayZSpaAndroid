...
public class MainActivity extends AppCompatActivity {
    ...
    private View buildMain(){
        ...
        LinearLayout refreshLp=new LinearLayout.LayoutParams(0,dp(46),1); refreshLp.setMargins(dp(6),0,0,0);
    ...
    private void searchWeatherInline(){
        ...
        new Thread(()->{try{
            ...
            JSONObject o=new JSONObject(body); org.json.JSONArray a=o.optJSONArray("results");
            runOnUiThread(()->{
                ...
                for(int i=0;i<a.length();i++){JSONObject x=a.optJSONObject(i);if(x==null)continue;String label=x.optString("name","");String admin=x.optString("admin1","");if(!admin.isEmpty())label+=" · "+admin;double lat=x.optDouble("latitude"),lon=x.optDouble("longitude");MaterialButton item=bigButton(label);item.setTextSize(14);item.setGravity(Gravity.CENTER_VERTICAL|Gravity.LEFT);item.setPadding(dp(14),0,dp(10),0);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,dp(48));lp.setMargins(0,dp(4),0,0);weatherInlineResultsContainer.addView(item,lp);item.setOnClickListener(v->{saveWeatherLocation(label,lat,lon);weatherInlineQuery.setText(label);weatherInlineQuery.setSelection(weatherInlineQuery.length());weatherInlineResult.setText("✓ Wybrano: "+label);weatherInlineResultsContainer.removeAllViews();});}
            });
