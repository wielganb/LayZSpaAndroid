...
public class MainActivity extends AppCompatActivity {
    ...
    private Spinner spinner(String[] items,int selected){Spinner s=new Spinner(this);android.graphics.drawable.GradientDrawable sbg=new android.graphics.drawable.GradientDrawable();sbg.setColor(CARD2);sbg.setCornerRadius(dp(18));sbg.setStroke(dp(1),BORDER);s.setBackground(sbg);ArrayAdapter<String>a=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,items){public View getView(int p,View c,ViewGroup parent){TextView v=(TextView)super.getView(p,c,parent);v.setTextColor(TEXT);v.setTextSize(15);v.setGravity(Gravity.CENTER_VERTICAL);v.setPadding(dp(16),0,dp(12),0);return v;}public View getDropDownView(int p,View c,ViewGroup parent){TextView v=(TextView)super.getDropDownView(p,c,parent);v.setTextColor(TEXT);v.setTextSize(16);v.setPadding(dp(18),dp(18),dp(12),dp(18));v.setBackgroundColor(CARD);return v;}};s.setAdapter(a);s.setSelection(selected);return s;}
    ...
    private View buildSettings(){ScrollView sc=new ScrollView(this);sc.setFillViewport(true);LinearLayout root=vertical();root.setPadding(dp(8),dp(8),dp(8),dp(30));sc.addView(root);
        ...
        root.addView(buildThemeSettings());gap(root,dp(10));root.addView(buildWeatherSettings());return sc;}
    ...
    private void updateSessionTimer(){if(sessionTimerView==null||sessionButton==null)return;long now=System.currentTimeMillis();if(sessionEndAt>now){sessionTimerView.setText(tr("remaining")+"  "+formatSession(sessionEndAt-now));sessionTimerView.setTextColor(RED);sessionButton.setText(sessionDurationMinutes+" "+tr("minutes"));handler.removeCallbacks(sessionTick);handler.postDelayed(sessionTick,1000);}else if(sessionEndAt!=0L){sessionEndAt=0L;saveSession();sessionTimerView.setText(tr("sessionReady"));sessionTimerView.setTextColor(MUTED);sessionButton.setText(sessionDurationMinutes+" "+tr("minutes"));/* Sesja dotyczy czasu kąpieli/bąbelków. Nigdy nie wyłączamy automatycznie grzania, ponieważ użytkownik może chcieć dogrzać wodę bez aktywnej sesji. */if(airState==1){send(2,0);airState=0;}updateControlLabels();}else{sessionTimerView.setText(tr("sessionReady"));sessionTimerView.setTextColor(MUTED);sessionButton.setText(sessionDurationMinutes+" "+tr("minutes"));}updateSessionQuickButtons();}
