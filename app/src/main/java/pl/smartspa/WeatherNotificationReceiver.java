...
public class WeatherNotificationReceiver extends BroadcastReceiver {
    ...
    private void run(Context c){
        try{
            ...
            Response resp=new OkHttpClient().newCall(new Request.Builder().url(u).build()).execute(); String body=resp.body()!=null?resp.body().string():""; JSONObject o=new JSONObject(body),cur=o.getJSONObject("current"); int out=(int)Math.round(cur.optDouble("temperature_2m",0)); int code=cur.optInt("weather_code",0); int rec=calc(out,code); String desc=desc(code);
    ...
    private int calc(int out,int code){int r;if(out<=7)r=38;else if(out<=20)r=37;else if(out<=27)r=36;else r=35;if(code>=51&&code<=86)r=Math.min(40,r+1);return Math.max(20,Math.min(40,r));}
    private String desc(int c){if(c==0)return"bezchmurnie";if(c<=3)return"częściowe zachmurzenie";if(c<=48)return"mgła lub zachmurzenie";if(c<=55)return"mżawka";if(c<=67)return"deszcz";if(c<=77)return"śnieg";if(c<=82)return"przelotne opady";if(c<=86)return"opady śniegu";return"burza";}
}
